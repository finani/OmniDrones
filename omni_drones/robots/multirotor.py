...
class Rotor(ActuatorBase):
    ...
    def __init__(
        ...
        with torch.device(self.device):
            ...
            self.kf_normalized = resolve(self.cfg.kf) * self.max_rotor_speed.square()
            self.km_normalized = resolve(self.cfg.km) * self.max_rotor_speed.square()
    ...
    def rotor_compute(self):
        ...
        throttle_square = self.throttle.square()
        thrusts = self.kf_normalized * throttle_square
        moments = self.km_normalized * throttle_square * -self.rotor_direction
